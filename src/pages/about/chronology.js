...
const Chronology = () => {
  ...
  useEffect(() => {
    ...
    const setDemoData = () => {
      const demoData = [
        { year: '1947', date: '4月1日', content: '久留米市野球連盟結成' },
  ...
  const staticContent = {
    ...
    body: `
      <h2>連盟の歩み</h2>
      <p>久留米市野球連盟は、地域のスポーツ振興と青少年の健全育成を目的に活動を続けてまいりました。</p>
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>{content.title || '久留米市野球連盟の沿革'}</title>
      ...
      <main className={styles.main}>
        ...
        <article className={styles.article}>
          {error ? (
            ...
            <>
              <h2 className={styles.articleTitle}>{content.title || '久留米市野球連盟の沿革'}</h2>
              <div 
